...
var EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
function parseContactToEmails(value) {
  if (!value) return [];
  return [...new Set(value.split(",").map((entry) => entry.trim()).filter((entry) => EMAIL_RE.test(entry)))];
}
__name(parseContactToEmails, "parseContactToEmails");
...
// .wrangler/tmp/bundle-sVY6zH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-sVY6zH/middleware-loader.entry.ts
